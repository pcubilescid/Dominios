        ...
        for line in f :# para line dentro de fichero
            ...
            if line[0:2] == 'OX': # si la line entre la posicion 0 y 2 es igual a OX
                ...
                if id_tax == tax_id: # si el id_tax es igual al tax_id (introducido por el usuario)
                    cabecera = "> " + ac_number + "; " + id + "; " + id_tax + "; " + descripcion + "\n\n" # la cabecera es igual al accession number principal de la entrada, el identificador único de la entrada, el identificador taxonómico del organismo, y la descripción larga recomendada, empezando por el caracter '> ' y separando cada punto con ;
                    resultado += cabecera + secuencia + "\n" # resultado es += a la cabecera mas la secuencia
